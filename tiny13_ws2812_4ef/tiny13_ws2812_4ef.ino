...
void wsClear(void) { for (k = 0; k < NUM_BYTES; k++) { leds[k] = 0; } }
...
// *************************** Колесо **************************
void wsWeel(void) {
	for (k = 0; k < NUM_LEDS; k++) { wsH2RGB(hue + k * 11, k, fade); }
	hue -= 2;
	addFade();
}
...
// ****************************** wsBeat ******************************
uint8_t pos = 0, pinc = 4;
#define BEAT_START 90
#define BEAT_STEP (210 / NUM_LEDS)
void wsBeat(void) {
	wsClear();
	for (k = 0; k < pos; k++) { wsH2RGB(BEAT_STEP * k + BEAT_START, k, fade); }
	addFade();
	pos += pinc;
	if (pos > (NUM_LEDS - 1)) { pinc = 255; }
	else if (pos < 1) { pinc = 4; }
}

// ****************************** КОНФЕТТИ ******************************
#define TRACK_STEP 40
void wsSparkles(void) {
	for (k = 0; k < NUM_BYTES; k++) { leds[k] = wsSub(leds[k], TRACK_STEP); }
	k = random8_1(NUM_LEDS);
	if ((leds[k*3] + leds[k*3+1] + leds[k*3+2]) == 0) { wsH2RGB(random8(), k, random8_2(100, 255)); }
}
...
uint8_t isTouch(void) {
	if (scnt) { scnt--; }
		...
		uint16_t v = TouchRead();
		if(v > sens) { sens++; }
		else { sens--; }

		if (v > (sens+TOUCH_THRESHOLD)) { sens++; scnt = 10; return 1; }
	}
	return 0;
}
...
/*__attribute__((section(".init9"))) */int main(void) {
	...
	while (1) {
		if (isTouch()) { if ((++idx) == N_EFF) { idx = 0; } }    

		switch(idx) {
			case 0: wsColors(); break;
			case 1: wsSparkles(); break;
			case 2: wsWeel(); break;
			default: wsBeat(); break;
		}
